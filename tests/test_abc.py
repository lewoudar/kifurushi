...
from kifurushi.abc import CommonField, Field, VariableStringField
...
class TestCommonField:
    ...
    @pytest.mark.parametrize('arguments', [{'name': 4, 'default': 2}, {'name': 'foo', 'default': 2, 'format': 4}])
...
class TestVariableStringField:
    ...
    # noinspection PyTypeChecker
    @pytest.mark.parametrize(('default', 'decode'), [(4.3, True), (4.3, False)])
    ...
    @pytest.mark.parametrize(('default', 'decode'), [(b'kifurushi', False), ('kifurushi', True)])
    ...
    def test_should_raise_error_when_default_length_is_greater_than_max_length(self):
        ...
        assert 'default must be less or equal than maximum length (6)' == str(exc_info.value)
    ...
    @pytest.mark.parametrize(('default', 'decode'), [('apple', True), (b'apple', False)])
    ...
    @pytest.mark.parametrize(('default', 'decode'), [('apple', True), (b'apple', False), (bytearray(b'apple'), False)])
    ...
    @pytest.mark.parametrize(('value', 'decode'), [('b' * 21, True), (b'b' * 21, False)])
    ...
    @pytest.mark.parametrize(('value', 'decode'), [('b' * 20, True), (b'b' * 20, False), (bytearray(b'b' * 20), False)])
    ...
    @pytest.mark.parametrize(('value', 'decode'), [('banana', True), (b'banana', False), (bytearray(b'banana'), False)])
    ...
    @pytest.mark.parametrize(
        ...
        [('banana', 'apple', True), (b'banana', b'apple', False), (bytearray(b'banana'), bytearray(b'apple'), False)],
    )
    ...
    @pytest.mark.parametrize(
        ('default', 'value', 'decode'),
        [('apple', 'banana', True), (b'apple', b'banana', False), (bytearray(b'apple'), bytearray(b'banana'), False)],
    )
