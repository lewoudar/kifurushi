...
out_of_boundaries_parametrize = pytest.mark.parametrize(
    ('field_class', 'left', 'right'),
    [
...
size_format_parametrize = pytest.mark.parametrize(('size', 'format_'), [(4, 'B'), (8, 'H'), (16, 'I'), (32, 'Q')])
...
class TestNumericField:
    ...
    @pytest.mark.parametrize(
        ('field_class', 'size', 'struct_format'),
        [
            (ByteField, 1, '!B'),
            (SignedByteField, 1, '!b'),
            (ShortField, 2, '!H'),
...
class TestEnumFields:
    ...
    @pytest.mark.parametrize('enumeration', [{'hello': 2}, [(2, 'hello')], {2: 3.4}])
    @pytest.mark.parametrize(
        'field_class',
        [
            ByteEnumField,
    ...
    def test_should_raise_error_when_key_value_is_out_of_field_boundaries(self, field_class, left, right):
        for value in [left, right]:
            ...
            assert (f'all keys in enumeration attribute must be ' f'between {left + 1} and {right - 1}') == str(
                exc_info.value
            )

    # noinspection PyTypeChecker
    @pytest.mark.parametrize('enumeration', [{1: 'mickey', 2: 'minnie'}, enum.Enum('Disney', 'mickey minnie')])
    @pytest.mark.parametrize(
        'field_class',
        [
            ByteEnumField,
...
class TestFixedStringField:
    ...
    @pytest.mark.parametrize(('default', 'decode'), [('hello', True), (b'hello', False), (bytearray(b'hello'), False)])
    ...
    @pytest.mark.parametrize(('default', 'decode'), [('h' * 8, True), (b'h' * 8, False), (bytearray(b'h' * 8), False)])
    ...
    @pytest.mark.parametrize(
        ('default', 'value', 'decode'), [('h' * 8, 4, True), (b'h' * 8, 4, False), (bytearray(b'h' * 8), 4, False)]
    )
    ...
    @pytest.mark.parametrize(
        ('default', 'value', 'decode'),
        [('h' * 8, 'b' * 8, True), (b'h' * 8, b'b' * 8, False), (bytearray(b'h' * 8), bytearray(b'b' * 8), False)],
    )
    ...
    @pytest.mark.parametrize(('default', 'value', 'decode'), [('h' * 8, 'b' * 8, True), (b'h' * 8, b'b' * 8, False)])
    ...
    @pytest.mark.parametrize(('default', 'decode'), [('h' * 8, True), (b'h' * 8, False)])
...
class TestFieldPart:
    ...
    @pytest.mark.parametrize('enumeration', [{1: 'MF', 2: 'DF', 4: 'reserved'}, enum.IntFlag('Flag', 'MF DF reserved')])
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                {1: 'MF', 4: 'reserved', 20: 'DF', 17: 'danger'},
                'FieldPart(name=flags, default=DF, value=danger)',
                False,
            ),
...
class TestBitsField:
    ...
    @pytest.mark.parametrize(('parts', 'error'), [({'foo': 2}, TypeError), ([], ValueError)])
    ...
    @pytest.mark.parametrize(('size_1', 'size_2'), [(3, 4), (4, 5)])
    ...
    @pytest.mark.parametrize(('value_1', 'value_2', 'hexadecimal'), [(4, 5, False), ('0x4', '0x5', True)])
    def test_should_correctly_represent_bits_field(self, value_1, value_2, hexadecimal):
        ...
        assert (
            f'BitsField(FieldPart(name=version, default={value_1}, value={value_1}),'
            f' FieldPart(name=IHL, default={value_2}, value={value_2}))'
        ) == repr(field)
    ...
    @pytest.mark.parametrize(
        ('size', 'format_', 'value_1', 'value_2'),
        [(4, 'B', -1, 2**4), (8, 'H', -1, 2**8), (16, 'I', -1, 2**16), (32, 'Q', -1, 2**32)],
    )
    ...
    @pytest.mark.parametrize(
        ('size', 'format_', 'right_value'),
        [(4, 'B', RIGHT_BYTE), (8, 'H', RIGHT_SHORT), (16, 'I', RIGHT_INT), (32, 'Q', RIGHT_LONG)],
    )
    ...
    @pytest.mark.parametrize(('size', 'format_'), [(16, 'I'), (32, 'Q')])
    ...
    def test_should_raise_error_when_trying_to_access_field_part_giving_a_wrong_name(self, size, format_):
        ...
        assert "'no field part was found with name foo'" == str(exc_info.value)
...
class TestSpecializedBitsFields:
    ...
    @pytest.mark.parametrize(
        ('bit_class', 'size', 'format_'),
        [(ByteBitsField, 4, 'B'), (ShortBitsField, 8, 'H'), (IntBitsField, 16, 'I'), (LongBitsField, 32, 'Q')],
    )
...
class TestConditionalField:
    ...
    @pytest.mark.parametrize('arguments', [('foo', lambda x: x), (ShortField('foo', 2), 'foo')])
    ...
    @pytest.mark.parametrize(('value', 'expected_data'), [(2, b'\x00\x02'), (1, b'')])
